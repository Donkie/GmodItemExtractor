...
{
    class GMAFile : IDisposable
    {
        private GMAHeader _header;
        private readonly GMAReader _reader;
        private readonly Dictionary<int, byte[]> _fileDataCache;

        public string Name => _header.Name;
        public GMAFileHeader[] Files => _header.Files;
        ...
        public GMAFileHeader? GetFileByPath(string path)
        {
            List<GMAFileHeader> li = Files.Where(f => PathEqual(f.Path, path)).ToList();
            if (li.Count == 0)
                return null;
            return li[0];
        }

        public GMAFileHeader[] GetFilesByPaths(string[] paths)
        {
            return Files.Where(f1 => paths.Any(f2 => PathEqual(f1.Path, f2))).ToArray();
        }
        ...
        {
            path1 = Program.CleanPath(path1);
            path2 = Program.CleanPath(path2);
